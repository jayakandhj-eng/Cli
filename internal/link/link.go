...
import (
	...
	cliConfig "github.com/Indobase/cli/pkg/config"
...
func linkPostgrest(ctx context.Context, projectRef string) error {
	resp, err := utils.GetIndobase().V1GetPostgrestServiceConfigWithResponse(ctx, projectRef)
	if err != nil {
...
func linkGotrue(ctx context.Context, projectRef string) error {
	resp, err := utils.GetIndobase().V1GetAuthServiceConfigWithResponse(ctx, projectRef)
	if err != nil {
...
func linkStorage(ctx context.Context, projectRef string, fsys afero.Fs) error {
	resp, err := utils.GetIndobase().V1GetStorageConfigWithResponse(ctx, projectRef)
	if err != nil {
...
func linkDatabaseSettings(ctx context.Context, projectRef string) error {
	resp, err := utils.GetIndobase().V1GetPostgresConfigWithResponse(ctx, projectRef)
	if err != nil {
...
func linkNetworkRestrictions(ctx context.Context, projectRef string) error {
	resp, err := utils.GetIndobase().V1GetNetworkRestrictionsWithResponse(ctx, projectRef)
	if err != nil {
...
func checkRemoteProjectStatus(ctx context.Context, projectRef string, fsys afero.Fs) error {
	resp, err := utils.GetIndobase().V1GetProjectWithResponse(ctx, projectRef)
	if err != nil {
	...
	case api.V1ProjectWithDatabaseResponseStatusINACTIVE:
		utils.CmdSuggestion = fmt.Sprintf("An admin must unpause it from the Indobase dashboard at %s", utils.Aqua(fmt.Sprintf("%s/project/%s", utils.GetIndobaseDashboardURL(), projectRef)))
