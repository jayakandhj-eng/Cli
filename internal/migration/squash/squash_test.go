...
func TestSquashCommand(t *testing.T) {
	...
	t.Run("baselines migration history", func(t *testing.T) {
		...
		defer conn.Close(t)
		helper.MockMigrationHistory(conn).
			Query(fmt.Sprintf("DELETE FROM Indobase_migrations.schema_migrations WHERE version <=  '0' ;INSERT INTO Indobase_migrations.schema_migrations(version, name, statements) VALUES( '0' ,  'init' ,  '{%s}' )", sql)).
			Reply("INSERT 0 1")
		// Run test
...
func TestBaselineMigration(t *testing.T) {
	...
	t.Run("throws error on query failure", func(t *testing.T) {
		...
		defer conn.Close(t)
		helper.MockMigrationHistory(conn).
			Query(fmt.Sprintf("DELETE FROM Indobase_migrations.schema_migrations WHERE version <=  '%[1]s' ;INSERT INTO Indobase_migrations.schema_migrations(version, name, statements) VALUES( '%[1]s' ,  'init' ,  null )", "0")).
			ReplyError(pgerrcode.InsufficientPrivilege, "permission denied for relation Indobase_migrations")
		// Run test
		err := baselineMigrations(context.Background(), dbConfig, "0", fsys, conn.Intercept, func(cc *pgx.ConnConfig) {
			cc.PreferSimpleProtocol = true
		})
		// Check error
		assert.ErrorContains(t, err, `ERROR: permission denied for relation Indobase_migrations (SQLSTATE 42501)`)
	})
...
func TestLineByLine(t *testing.T) {
	t.Run("diffs output from pg_dump", func(t *testing.T) {
		...
		// Check error
		assert.NoError(t, err)
		expectedString := strings.ReplaceAll(string(expected), "\r\n", "\n")
		outString := strings.ReplaceAll(out.String(), "\r\n", "\n")
		assert.Equal(t, expectedString, outString)
	})
