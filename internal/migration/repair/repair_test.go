...
func TestRepairCommand(t *testing.T) {
	...
	t.Run("throws error on insert failure", func(t *testing.T) {
		...
		helper.MockMigrationHistory(conn).
			...
			ReplyError(pgerrcode.DuplicateObject, `relation "Indobase_migrations.schema_migrations" does not exist`)
		// Run test
		err := Run(context.Background(), dbConfig, []string{"0"}, Applied, fsys, conn.Intercept)
		// Check error
		assert.ErrorContains(t, err, `ERROR: relation "Indobase_migrations.schema_migrations" does not exist (SQLSTATE 42710)`)
	})
}
