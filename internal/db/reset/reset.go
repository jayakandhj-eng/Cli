...
import (
	...
	"github.com/Indobase/cli/internal/migration/apply"
	"github.com/Indobase/cli/internal/migration/down"
	"github.com/Indobase/cli/internal/migration/list"
...
func Run(ctx context.Context, version string, last uint, config pgconn.Config, fsys afero.Fs, options ...func(*pgx.ConnConfig)) error {
	...
	// Seed objects from Indobase/buckets directory
	...
	fmt.Fprintln(os.Stderr, "Finished "+utils.Aqua("Indobase db reset")+" on branch "+utils.Aqua(branch)+".")
	return nil
}
...
func recreateDatabase(ctx context.Context, options ...func(*pgx.ConnConfig)) error {
	...
	sql := migration.MigrationFile{
		Statements: []string{
			...
			"DROP DATABASE IF EXISTS _Indobase WITH (FORCE)",
...
const (
	TERMINATE_BACKENDS      = "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname IN ('postgres', '_Indobase')"
	COUNT_REPLICATION_SLOTS = "SELECT COUNT(*) FROM pg_replication_slots WHERE database IN ('postgres', '_Indobase')"
)
...
func DisconnectClients(ctx context.Context, conn *pgx.Conn) error {
	...
	disconn := migration.MigrationFile{
		Statements: []string{
			...
			"ALTER DATABASE _Indobase ALLOW_CONNECTIONS false",
			TERMINATE_BACKENDS,
		},
	}
