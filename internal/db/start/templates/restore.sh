...
cat "/etc/backup.sql" \
| grep 'CREATE ROLE' \
| grep -v 'Indobase_admin' \
| sed -E 's/^(CREATE ROLE postgres);/\1 WITH SUPERUSER;/' \
| psql -v ON_ERROR_STOP=1 --no-password --no-psqlrc -U Indobase_admin
...
if [ -e "$postinit" ]; then
    ...
    psql -v ON_ERROR_STOP=1 --no-password --no-psqlrc -U Indobase_admin -f "$postinit"
fi
