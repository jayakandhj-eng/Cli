...
import (
	...
	"github.com/Indobase/cli/pkg/vault"
)
...
var (
	...
	//go:embed templates/_Indobase.sql
	_IndobaseSchema string
...
cat <<'EOF' > /docker-entrypoint-initdb.d/Indobase_schema.sql && \
cat <<'EOF' >> /etc/postgresql/postgresql.conf && \
docker-entrypoint.sh postgres -D /etc/postgresql ` + strings.Join(args, " ") + `
` + _IndobaseSchema + `
EOF
...
EOF`}
	...
	if utils.NoBackupVolume {
		...
		utils.CmdSuggestion = fmt.Sprintf("Run %s to remove existing docker volumes.", utils.Aqua("Indobase stop --no-backup"))
...
func initStorageJob(host string) utils.DockerJob {
	return utils.DockerJob{
		...
		Env: []string{
			...
			fmt.Sprintf("DATABASE_URL=postgresql://Indobase_storage_admin:%s@%s:5432/postgres", utils.Config.Db.Password, host),
...
func initAuthJob(host string) utils.DockerJob {
	return utils.DockerJob{
		...
		Env: []string{
			...
			fmt.Sprintf("GOTRUE_DB_DATABASE_URL=postgresql://Indobase_auth_admin:%s@%s:5432/postgres", utils.Config.Db.Password, host),
