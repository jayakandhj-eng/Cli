...
import (
	...
	"github.com/Indobase/cli/internal/migration/format"
	"github.com/Indobase/cli/internal/migration/list"
	"github.com/Indobase/cli/internal/migration/new"
...
func run(ctx context.Context, schema []string, path string, conn *pgx.Conn, fsys afero.Fs) error {
	...
	// 1. Assert `Indobase/migrations` and `schema_migrations` are in sync.
...
func suggestMigrationRepair(extraRemote, extraLocal []string) string {
	...
	for _, version := range extraRemote {
		result += fmt.Sprintln(utils.Bold("Indobase migration repair --status reverted " + version))
	}
	for _, version := range extraLocal {
		result += fmt.Sprintln(utils.Bold("Indobase migration repair --status applied " + version))
	}
	return result
}
