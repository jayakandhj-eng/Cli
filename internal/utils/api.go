...
import (
	...
	"github.com/Indobase/cli/internal/utils/cloudflare"
	Indobase "github.com/Indobase/cli/pkg/api"
...
var (
	...
	apiClient  *Indobase.ClientWithResponses
...
func GetIndobase() *Indobase.ClientWithResponses {
	clientOnce.Do(func() {
		...
		apiClient, err = Indobase.NewClientWithResponses(
			GetIndobaseAPIHost(),
			Indobase.WithRequestEditorFn(func(ctx context.Context, req *http.Request) error {
				req.Header.Set("Authorization", "Bearer "+token)
				req.Header.Set("User-Agent", "IndobaseCLI/"+Version)
				return nil
			}),
		)
		if err != nil {
...
func GetIndobaseAPIHost() string {
	return CurrentProfile.APIURL
}

func GetIndobaseDashboardURL() string {
	return CurrentProfile.DashboardURL
}

func GetIndobaseHost(projectRef string) string {
	return fmt.Sprintf("%s.%s", projectRef, CurrentProfile.ProjectHost)
}

func GetIndobaseDbHost(projectRef string) string {
	return "db." + GetIndobaseHost(projectRef)
}
