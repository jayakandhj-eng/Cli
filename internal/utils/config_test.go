...
func TestGetId(t *testing.T) {
	t.Run("generates container id", func(t *testing.T) {
		...
		assert.Equal(t, "Indobase_test-service_test-project", id)
	})
}
...
func TestUpdateDockerIds(t *testing.T) {
	t.Run("updates all container ids", func(t *testing.T) {
		...
		assert.Equal(t, "Indobase_db_test-project", DbId)
		assert.Equal(t, "Indobase_kong_test-project", KongId)
		assert.Equal(t, "Indobase_auth_test-project", GotrueId)
		assert.Equal(t, "Indobase_inbucket_test-project", InbucketId)
		assert.Equal(t, "Indobase_realtime_test-project", RealtimeId)
		assert.Equal(t, "Indobase_rest_test-project", RestId)
		assert.Equal(t, "Indobase_storage_test-project", StorageId)
		assert.Equal(t, "Indobase_imgproxy_test-project", ImgProxyId)
		assert.Equal(t, "Indobase_differ_test-project", DifferId)
		assert.Equal(t, "Indobase_pg_meta_test-project", PgmetaId)
		assert.Equal(t, "Indobase_studio_test-project", StudioId)
		assert.Equal(t, "Indobase_edge_runtime_test-project", EdgeRuntimeId)
		assert.Equal(t, "Indobase_analytics_test-project", LogflareId)
		assert.Equal(t, "Indobase_vector_test-project", VectorId)
		assert.Equal(t, "Indobase_pooler_test-project", PoolerId)
	})
	...
	t.Run("generates network id if not set", func(t *testing.T) {
		...
		assert.Equal(t, "Indobase_network_test-project", NetId)
	})
}
