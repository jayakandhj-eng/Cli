...
func TestProjectRoot(t *testing.T) {
	...
	t.Run("stops at closest parent", func(t *testing.T) {
		// Setup in-memory fs
		fsys := afero.NewMemMapFs()
		_, err := fsys.Create(filepath.Join(root, "Indobase", ConfigPath))
		require.NoError(t, err)
		// Run test
		cwd := filepath.Join(root, "Indobase", "Indobase", "functions")
		path := getProjectRoot(cwd, fsys)
		// Check error
		assert.Equal(t, filepath.Join(root, "Indobase"), path)
	})
	...
	t.Run("ignores error if path is not directory", func(t *testing.T) {
		...
		require.NoError(t, err)
		// Setup in-memory fs
		fsys := &MockFs{DenyPath: filepath.Join(cwd, "Indobase")}
		// Run test
		path := getProjectRoot(cwd, fsys)
		// Check error
		assert.NoError(t, err)
...
func TestShortContainerImageName(t *testing.T) {
	t.Run("extracts short name from image", func(t *testing.T) {
		input := "registry.Indobase.com/postgres:15.1.0.99"
		expected := "postgres"
