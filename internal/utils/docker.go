...
const (
	...
	CliProjectLabel     = "com.Indobase.cli.project"
...
// Defaults to Indobase public ECR for faster image pull
...
func GetRegistryImageUrl(imageName string) string {
	...
	return registry + "/Indobase/" + imageName
}
			...
			for _, p := range c.Ports {
				if fmt.Sprintf("%s:%d", p.IP, p.PublicPort) == hostPort {
					if project, ok := c.Labels[CliProjectLabel]; ok {
						return "\nTry stopping the running project with " + Aqua("Indobase stop --project-id "+project)
					} else {
						name := c.ID
						if len(c.Names) > 0 {
							name = c.Names[0]
