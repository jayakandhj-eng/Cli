...
import "../../../Indobase/tests/index.ts"
import "./child/index.ts"`
		require.NoError(t, WriteFile("/app/Indobase/functions/hello/index.ts", []byte(entrypoint), fsys))
		require.NoError(t, WriteFile("/tmp/index.ts", []byte{}, fsys))
		require.NoError(t, WriteFile("/app/Indobase/functions/common/index.ts", []byte{}, fsys))
		require.NoError(t, WriteFile("/app/Indobase/tests/index.ts", []byte{}, fsys))
		require.NoError(t, WriteFile("/app/Indobase/functions/hello/child/index.ts", []byte{}, fsys))
		// Run test
		mods, err := BindHostModules("/app", "Indobase/functions/hello/index.ts", "", fsys)
		// Check error
		assert.NoError(t, err)
		if runtime.GOOS == "windows" {
			assert.ElementsMatch(t, mods, []string{
				"\\app\\Indobase\\functions\\hello\\index.ts:/app/Indobase/functions/hello/index.ts:ro",
				"\\tmp\\index.ts:/tmp/index.ts:ro",
				"\\app\\Indobase\\functions\\common\\index.ts:/app/Indobase/functions/common/index.ts:ro",
				"\\app\\Indobase\\tests\\index.ts:/app/Indobase/tests/index.ts:ro",
				"\\app\\Indobase\\functions\\hello\\child\\index.ts:/app/Indobase/functions/hello/child/index.ts:ro",
			})
		} else {
			assert.ElementsMatch(t, mods, []string{
				"/app/Indobase/functions/hello/index.ts:/app/Indobase/functions/hello/index.ts:ro",
				...
				"/app/Indobase/functions/common/index.ts:/app/Indobase/functions/common/index.ts:ro",
				"/app/Indobase/tests/index.ts:/app/Indobase/tests/index.ts:ro",
				"/app/Indobase/functions/hello/child/index.ts:/app/Indobase/functions/hello/child/index.ts:ro",
			})
		}
	})
...
func TestGetDenoPath(t *testing.T) {
	...
	t.Run("returns default path", func(t *testing.T) {
		...
		expected := filepath.Join(home, ".Indobase", "deno")
		if runtime.GOOS == "windows" {
			expected += ".exe"
		}
...
func TestCopyDenoScripts(t *testing.T) {
	t.Run("copies deno scripts", func(t *testing.T) {
		...
		home, err := os.UserHomeDir()
		require.NoError(t, err)
		denoDir := filepath.Join(home, ".Indobase")
	...
	t.Run("skips copying unmodified scripts", func(t *testing.T) {
		...
		home, err := os.UserHomeDir()
		require.NoError(t, err)
		scriptDir := filepath.Join(home, ".Indobase", "denos")
