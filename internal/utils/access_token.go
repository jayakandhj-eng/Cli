...
var (
	...
	ErrMissingToken    = errors.Errorf("Access token not provided. Supply an access token by running %s or setting the Indobase_ACCESS_TOKEN environment variable.", Aqua("Indobase login"))
...
func loadAccessToken(fsys afero.Fs) (string, error) {
	...
	if accessToken := os.Getenv("Indobase_ACCESS_TOKEN"); accessToken != "" {
...
func getAccessTokenPath() (string, error) {
	...
	return filepath.Join(home, ".Indobase", AccessTokenKey), nil
}
