...
func TestParseDatabaseConfig(t *testing.T) {
	...
	t.Run("parses linked connection", func(t *testing.T) {
		...
		dbURL := fmt.Sprintf("postgres://postgres:postgres@db.%s.Indobase.co:6543/postgres", project)
		...
		assert.NoError(t, err)
		assert.True(t, strings.HasPrefix(DbConfig.Host, utils.GetIndobaseDbHost(project)))
	})
}
