...
import (
	...
	"github.com/Indobase/cli/internal/utils/credentials"
...
func ParseDatabaseConfig(ctx context.Context, flagSet *pflag.FlagSet, fsys afero.Fs) error {
	...
	case proxy:
		...
		DbConfig.Host = utils.GetIndobaseAPIHost()
		DbConfig.Port = 443
		DbConfig.User = "postgres"
...
const suggestEnvVar = "Connect to your database by setting the env var: Indobase_DB_PASSWORD"

func NewDbConfigWithPassword(ctx context.Context, projectRef string) (pgconn.Config, error) {
	config := pgconn.Config{
		Host:     utils.GetIndobaseDbHost(projectRef),
		Port:     5432,
		User:     "postgres",
	...
	if conn, err := d.DialContext(ctx, "tcp", addr); err == nil {
		...
		utils.CmdSuggestion = fmt.Sprintf("Run %s to setup IPv4 connection.", utils.Aqua("Indobase link --project-ref "+projectRef))
...
func initLoginRole(ctx context.Context, projectRef string, config *pgconn.Config) error {
	...
	resp, err := utils.GetIndobase().V1CreateLoginRoleWithResponse(ctx, projectRef, body)
	if err != nil {
...
func ListNetworkBans(ctx context.Context, projectRef string) ([]string, error) {
	resp, err := utils.GetIndobase().V1ListAllNetworkBansWithResponse(ctx, projectRef)
	if err != nil {
...
func UnbanIP(ctx context.Context, projectRef string, addrs ...string) error {
	...
	if resp, err := utils.GetIndobase().V1DeleteNetworkBansWithResponse(ctx, projectRef, body); err != nil {
