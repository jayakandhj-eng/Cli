...
import (
	...
	"github.com/Indobase/cli/internal/db/push"
	initBlank "github.com/Indobase/cli/internal/init"
	"github.com/Indobase/cli/internal/link"
	"github.com/Indobase/cli/internal/login"
	"github.com/Indobase/cli/internal/projects/apiKeys"
	"github.com/Indobase/cli/internal/projects/create"
...
func checkProjectHealth(ctx context.Context) error {
	...
	resp, err := utils.GetIndobase().V1GetServicesHealthWithResponse(ctx, flags.ProjectRef, &params)
	if err != nil {
		return err
	}
	if resp.JSON200 == nil {
...
const (
	Indobase_SERVICE_ROLE_KEY = "Indobase_SERVICE_ROLE_KEY"
	Indobase_ANON_KEY         = "Indobase_ANON_KEY"
	Indobase_URL              = "Indobase_URL"
	...
	NEXT_PUBLIC_Indobase_ANON_KEY = "NEXT_PUBLIC_Indobase_ANON_KEY"
	NEXT_PUBLIC_Indobase_URL      = "NEXT_PUBLIC_Indobase_URL"
	EXPO_PUBLIC_Indobase_ANON_KEY = "EXPO_PUBLIC_Indobase_ANON_KEY"
	EXPO_PUBLIC_Indobase_URL      = "EXPO_PUBLIC_Indobase_URL"
)
...
func writeDotEnv(keys []api.ApiKeyResponse, config pgconn.Config, fsys afero.Fs) error {
	...
	initial[Indobase_URL] = "https://" + utils.GetIndobaseHost(flags.ProjectRef)
	...
	for k, v := range envs {
		switch k {
		case Indobase_SERVICE_ROLE_KEY:
		case Indobase_ANON_KEY:
		case Indobase_URL:
		case POSTGRES_URL:
		// Derived keys
		...
		case NEXT_PUBLIC_Indobase_ANON_KEY:
			fallthrough
		case EXPO_PUBLIC_Indobase_ANON_KEY:
			initial[k] = initial[Indobase_ANON_KEY]
		case NEXT_PUBLIC_Indobase_URL:
			fallthrough
		case EXPO_PUBLIC_Indobase_URL:
			initial[k] = initial[Indobase_URL]
		default:
			initial[k] = v
		}
	}
...
func ListSamples(ctx context.Context, client *github.Client) ([]StarterTemplate, error) {
	owner := "Indobase-community"
	repo := "Indobase-samples"
	path := "samples.json"
	ref := "main"
