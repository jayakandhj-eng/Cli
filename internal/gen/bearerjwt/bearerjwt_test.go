...
func TestGenerateToken(t *testing.T) {
	...
	t.Run("mints custom JWT", func(t *testing.T) {
		...
		require.NoError(t, utils.WriteFile("Indobase/config.toml", []byte(`
			[auth]
			signing_keys_path = "./keys.json"
		`), fsys))
		testKey, err := json.Marshal([]config.JWK{*privateKeyECDSA})
		require.NoError(t, err)
		require.NoError(t, utils.WriteFile("Indobase/keys.json", testKey, fsys))
		// Run test
	...
	t.Run("throws error on missing key", func(t *testing.T) {
		...
		// Setup in-memory fs
		fsys := afero.NewMemMapFs()
		require.NoError(t, utils.WriteFile("Indobase/keys.json", []byte("[]"), fsys))
		require.NoError(t, utils.WriteFile("Indobase/config.toml", []byte(`
			[auth]
			signing_keys_path = "./keys.json"
		`), fsys))
