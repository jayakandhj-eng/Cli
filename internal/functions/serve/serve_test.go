...
func TestServeCommand(t *testing.T) {
	t.Run("serves all functions", func(t *testing.T) {
		...
		gock.New(utils.Docker.DaemonHost()).
			Get("/v" + utils.Docker.ClientVersion() + "/containers/Indobase_db_test/json").
			Reply(http.StatusOK).
			JSON(container.InspectResponse{})
		containerId := "Indobase_edge_runtime_test"
		gock.New(utils.Docker.DaemonHost()).
...
func TestServeFunctions(t *testing.T) {
	...
	t.Run("parses function config", func(t *testing.T) {
		...
		// Check error
		assert.NoError(t, err)
		normalizedBinds := make([]string, len(binds))
		for i, bind := range binds {
			normalizedBinds[i] = strings.ReplaceAll(bind, "\\", "/")
		}
		assert.ElementsMatch(t, []string{
			"Indobase_edge_runtime_test:/root/.cache/deno:rw",
			"/Indobase/functions/:/Indobase/functions/:ro",
		}, normalizedBinds)
		normalizedConfig := strings.ReplaceAll(configString, "\\", "/")
		assert.Equal(t, `{"hello":{"verifyJWT":true,"entrypointPath":"testdata/functions/hello/index.ts","staticFiles":["testdata/image.png"]}}`, normalizedConfig)
	})
}
