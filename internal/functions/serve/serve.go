...
func ServeFunctions(ctx context.Context, envFilePath string, noVerifyJWT *bool, importMapPath string, dbUrl string, runtimeOption RuntimeOption, fsys afero.Fs) error {
	...
	env = append(env,
		fmt.Sprintf("Indobase_URL=http://%s:8000", utils.KongAliases[0]),
		"Indobase_ANON_KEY="+utils.Config.Auth.AnonKey.Value,
		"Indobase_SERVICE_ROLE_KEY="+utils.Config.Auth.ServiceRoleKey.Value,
		"Indobase_DB_URL="+dbUrl,
		"Indobase_INTERNAL_JWT_SECRET="+utils.Config.Auth.JwtSecret.Value,
		"Indobase_INTERNAL_JWKS="+jwks,
		fmt.Sprintf("Indobase_INTERNAL_HOST_PORT=%d", utils.Config.Api.Port),
	)
	if viper.GetBool("DEBUG") {
		env = append(env, "Indobase_INTERNAL_DEBUG=true")
	}
	if runtimeOption.InspectMode != nil {
		env = append(env, "Indobase_INTERNAL_WALLCLOCK_LIMIT_SEC=0")
	}
	...
	env = append(env, "Indobase_INTERNAL_FUNCTIONS_CONFIG="+functionsConfigString)
