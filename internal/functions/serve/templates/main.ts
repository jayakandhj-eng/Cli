...
const HOST_PORT = Deno.env.get("Indobase_INTERNAL_HOST_PORT")!;
const JWT_SECRET = Deno.env.get("Indobase_INTERNAL_JWT_SECRET")!;
const JWKS_ENDPOINT = new URL('/auth/v1/.well-known/jwks.json', Deno.env.get("Indobase_URL")!)
const DEBUG = Deno.env.get("Indobase_INTERNAL_DEBUG") === "true";
const FUNCTIONS_CONFIG_STRING = Deno.env.get(
  "Indobase_INTERNAL_FUNCTIONS_CONFIG",
)!;

const WALLCLOCK_LIMIT_SEC = parseInt(
  Deno.env.get("Indobase_INTERNAL_WALLCLOCK_LIMIT_SEC"),
);
...
let jwks = (() => {
  try {
    ...
    return jose.createLocalJWKSet(JSON.parse(Deno.env.get('Indobase_INTERNAL_JWKS')));
  } catch (error) {
    return null
  }
})();
...
Deno.serve({
  handler: async (req: Request) => {
    ...
    // Ref: https://Indobase.com/docs/guides/functions/limits
    ...
    const envVars = Object.entries(envVarsObj)
      .filter(([name, _]) =>
        !EXCLUDED_ENVS.includes(name) && !name.startsWith("Indobase_INTERNAL_")
      );
