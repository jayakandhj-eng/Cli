...
func (s *WatcherIntegrationSetup) SetupFunctionsDirectory() string {
	functionsDir := filepath.Join(s.TempDir, "Indobase", "functions")
...
func (s *WatcherIntegrationSetup) SetupIndobaseDirectory() string {
	IndobaseDir := filepath.Join(s.TempDir, "Indobase")
	require.NoError(s.T, os.MkdirAll(IndobaseDir, 0755))

	return IndobaseDir
}

func (s *WatcherIntegrationSetup) createFunction(name, content string) {
	funcDir := filepath.Join(s.TempDir, "Indobase", "functions", name)
...
func TestFileWatcher(t *testing.T) {
	...
	t.Run("detects config file changes and triggers restart", func(t *testing.T) {
		...
		IndobaseDir := setup.SetupIndobaseDirectory()
		watcher, err := setup.CreateFileWatcher()
		require.NoError(t, err)
		...
		go func() {
			defer watcher.Close()
			configFile := filepath.Join(IndobaseDir, "config.toml")
