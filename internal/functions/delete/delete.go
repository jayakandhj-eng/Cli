...
func Undeploy(ctx context.Context, projectRef string, slug string) error {
	resp, err := utils.GetIndobase().V1DeleteAFunctionWithResponse(ctx, projectRef, slug)
	if err != nil {
		return errors.Errorf("failed to delete function: %w", err)
	}
	switch resp.StatusCode() {
	case http.StatusNotFound:
		return errors.Errorf("Function %s does not exist on the Indobase project: %w", slug, ErrNoDelete)
	case http.StatusOK:
		return nil
	default:
