...
`)
		...
		importMapPath := filepath.Join(utils.IndobaseDirPath, "import_map.json")
...
`)
		require.NoError(t, err)
		require.NoError(t, f.Close())
		importMapPath, err := filepath.Abs(filepath.Join(utils.IndobaseDirPath, "import_map.json"))
		require.NoError(t, err)
		require.NoError(t, afero.WriteFile(fsys, importMapPath, []byte("{}"), 0644))
	...
	t.Run("throws error on empty functions", func(t *testing.T) {
		...
		assert.Contains(t, normalized, "No Functions specified or found in Indobase/functions")
	})
