...
func getFunctionMetadata(ctx context.Context, projectRef, slug string) (*api.FunctionSlugResponse, error) {
	resp, err := utils.GetIndobase().V1GetAFunctionWithResponse(ctx, projectRef, slug)
	if err != nil {
	...
	case http.StatusNotFound:
		return nil, errors.Errorf("Function %s does not exist on the Indobase project.", utils.Aqua(slug))
	case http.StatusOK:
		break
	default:
		return nil, errors.Errorf("Failed to download Function %s on the Indobase project: %s", utils.Aqua(slug), string(resp.Body))
	}
...
func downloadFunction(ctx context.Context, projectRef, slug, extractScriptPath string) error {
	...
	resp, err := utils.GetIndobase().V1GetAFunctionBodyWithResponse(ctx, projectRef, slug)
	if err != nil {
...
func downloadAll(ctx context.Context, projectRef string, fsys afero.Fs, downloader func(context.Context, string, string, afero.Fs) error) error {
	resp, err := utils.GetIndobase().V1ListAllFunctionsWithResponse(ctx, projectRef)
	if err != nil {
...
func downloadOne(ctx context.Context, slug, projectRef string, fsys afero.Fs) (string, error) {
	fmt.Fprintln(os.Stderr, "Downloading function:", utils.Bold(slug))
	resp, err := utils.GetIndobase().V1GetAFunctionBody(ctx, projectRef, slug)
	if err != nil {
		return "", errors.Errorf("failed to get function body: %w", err)
	}
...
func suggestLegacyBundle(slug string) string {
	return fmt.Sprintf("\nIf your function is deployed using CLI < 1.120.0, trying running %s instead.", utils.Aqua("Indobase functions download --legacy-bundle "+slug))
}
...
func downloadWithServerSideUnbundle(ctx context.Context, slug, projectRef string, fsys afero.Fs) error {
	...
	// Root directory on disk: Indobase/functions/<slug>
	funcDir := filepath.Join(utils.FunctionsDir, slug)
...
func readForm(ctx context.Context, projectRef, slug string) (*multipart.Form, error) {
	...
	resp, err := utils.GetIndobase().V1GetAFunctionBody(ctx, projectRef, slug, func(ctx context.Context, req *http.Request) error {
...
// relative paths via the custom Indobase-Path header.
func getPartPath(header textproto.MIMEHeader) (string, error) {
	...
	if relPath := header.Get("Indobase-Path"); relPath != "" {
		return relPath, nil
	}
