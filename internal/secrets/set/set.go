...
func Run(ctx context.Context, projectRef, envFilePath string, args []string, fsys afero.Fs) error {
	...
	// 2. Set secret(s).
	resp, err := utils.GetIndobase().V1BulkCreateSecretsWithResponse(ctx, projectRef, secrets)
	if err != nil {
	...
	fmt.Println("Finished " + utils.Aqua("Indobase secrets set") + ".")
	return nil
}
...
func ListSecrets(envFilePath string, fsys afero.Fs, envArgs ...string) (api.CreateSecretBody, error) {
	...
	for name, value := range envMap {
		...
		if strings.HasPrefix(name, "Indobase_") {
			fmt.Fprintln(os.Stderr, "Env name cannot start with Indobase_, skipping: "+name)
			continue
		}
