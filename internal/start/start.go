...
import (
	...
	"github.com/Indobase/cli/internal/db/start"
	"github.com/Indobase/cli/internal/functions/serve"
	"github.com/Indobase/cli/internal/seed/buckets"
	"github.com/Indobase/cli/internal/services"
...
func Run(ctx context.Context, fsys afero.Fs, excludedContainers []string, ignoreHealthCheck bool) error {
	// Sanity checks.
	{
		...
		if err := utils.AssertIndobaseDbIsRunning(); err == nil {
			fmt.Fprintln(os.Stderr, utils.Aqua("Indobase start")+" is already running.")
...
func run(ctx context.Context, fsys afero.Fs, excludedContainers []string, dbConfig pgconn.Config, options ...func(*pgx.ConnConfig)) error {
	...
	if utils.Config.Analytics.Enabled && !isContainerExcluded(utils.Config.Analytics.Image, excluded) {
		...
		case config.LogflarePostgres:
			env = append(env,
				fmt.Sprintf("POSTGRES_BACKEND_URL=postgresql://%s:%s@%s:%d/%s", dbConfig.User, dbConfig.Password, dbConfig.Host, dbConfig.Port, "_Indobase"),
...
`},
	...
	if utils.Config.Analytics.Enabled && !isContainerExcluded(utils.Config.Analytics.VectorImage, excluded) {
		...
		case "npipe":
			const dockerDaemonNeededErr = "Analytics on Windows requires Docker daemon exposed on tcp://localhost:2375.\nSee https://Indobase.com/docs/guides/local-development/cli/getting-started?queryGroups=platform&platform=windows#running-Indobase-locally for more details."
	...
	if !isContainerExcluded(utils.Config.Api.KongImage, excluded) {
		...
		if err := kongConfigTemplate.Option("missingkey=error").Execute(&kongConfigBuf, kongConfig{
			...
			BearerToken: fmt.Sprintf(
				...
				// Legacy Indobase-js may set Authorization header to Bearer <apikey>. We must remove it
		...
		if _, err := utils.DockerStart(
			...
			container.Config{
				...
				Env: []string{
					...
					"KONG_DNS_ORDER=LAST,A,CNAME", // https://github.com/Indobase/cli/issues/14
...
`},
	...
	if utils.Config.Auth.Enabled && !isContainerExcluded(utils.Config.Auth.Image, excluded) {
		...
		env := []string{
			...
			fmt.Sprintf("GOTRUE_DB_DATABASE_URL=postgresql://Indobase_auth_admin:%s@%s:%d/%s", dbConfig.Password, dbConfig.Host, dbConfig.Port, dbConfig.Database),
		...
		if _, err := utils.DockerStart(
			...
			container.Config{
				...
				Env: []string{
					...
					fmt.Sprintf("DATABASE_URL=postgresql://Indobase_storage_admin:%s@%s:%d/%s", dbConfig.Password, dbConfig.Host, dbConfig.Port, dbConfig.Database),
					fmt.Sprintf("FILE_SIZE_LIMIT=%v", utils.Config.Storage.FileSizeLimit),
					"STORAGE_BACKEND=file",
					"FILE_STORAGE_BACKEND_PATH=" + dockerStoragePath,
					"TENANT_ID=stub",
					// TODO: https://github.com/Indobase/storage-api/issues/55
		...
		if _, err := utils.DockerStart(
			...
			container.Config{
				...
				Env: []string{
					...
					"Indobase_URL=http://" + utils.KongId + ":8000",
					"Indobase_PUBLIC_URL=" + utils.Config.Studio.ApiUrl,
					"AUTH_JWT_SECRET=" + utils.Config.Auth.JwtSecret.Value,
					"Indobase_ANON_KEY=" + utils.Config.Auth.AnonKey.Value,
					"Indobase_SERVICE_KEY=" + utils.Config.Auth.ServiceRoleKey.Value,
					"LOGFLARE_PRIVATE_ACCESS_TOKEN=" + utils.Config.Analytics.ApiKey,
		...
		if _, err := utils.DockerStart(
			...
			container.Config{
				...
				Env: []string{
					...
					fmt.Sprintf("DATABASE_URL=ecto://%s:%s@%s:%d/%s", dbConfig.User, dbConfig.Password, dbConfig.Host, dbConfig.Port, "_Indobase"),
