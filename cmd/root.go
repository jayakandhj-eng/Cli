...
import (
	...
	"github.com/Indobase/cli/internal/debug"
...
func promptLogin(fsys afero.Fs) error {
	if _, err := utils.LoadAccessTokenFS(fsys); err == utils.ErrMissingToken {
		utils.CmdSuggestion = fmt.Sprintf("Run %s first.", utils.Aqua("Indobase login"))
...
var (
	...
	rootCmd = &cobra.Command{
		Use:     "Indobase",
		Short:   "Indobase CLI " + utils.Version,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			apiHost, err := url.Parse(utils.GetIndobaseAPIHost())
			if err != nil {
				return err
			}
...
func checkUpgrade(ctx context.Context, fsys afero.Fs) (string, error) {
	if shouldFetchRelease(fsys) {
		...
		if exists, _ := afero.DirExists(fsys, utils.IndobaseDirPath); exists {
...
func suggestUpgrade(version string) string {
	const guide = "https://Indobase.com/docs/guides/cli/getting-started#updating-the-Indobase-cli"
	return fmt.Sprintf(`A new version of Indobase CLI is available: %s (currently installed v%s)
...
func recoverAndExit() {
	...
	if createTicket && len(utils.SentryDsn) > 0 {
		...
		if eventId != nil && sentry.Flush(2*time.Second) {
			...
			fmt.Fprintln(os.Stderr, "Quote the crash ID above when filing a bug report: https://github.com/Indobase/cli/issues/new/choose")
		}
	}
	os.Exit(1)
}
...
func init() {
	...
	flags.String("workdir", "", "path to a Indobase project directory")
	...
	flags.String("profile", "Indobase", "use a specific profile for connecting to Indobase API")
