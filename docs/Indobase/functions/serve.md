## Indobase-functions-serve

Serve all Functions locally.

`Indobase functions serve` command includes additional flags to assist developers in debugging Edge Functions via the v8 inspector protocol, allowing for debugging via Chrome DevTools, VS Code, and IntelliJ IDEA for example. Refer to the [docs guide](/docs/guides/functions/debugging-tools) for setup instructions.
...
Additionally, the following properties can be customized via `Indobase/config.toml` under `edge_runtime` section.
