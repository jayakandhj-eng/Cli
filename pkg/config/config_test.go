...
func TestConfigParsing(t *testing.T) {
	...
	t.Run("config file with environment variables", func(t *testing.T) {
		...
		// Setup in-memory fs
		fsys := fs.MapFS{
			"Indobase/config.toml":           &fs.MapFile{Data: testInitConfigEmbed},
			"Indobase/templates/invite.html": &fs.MapFile{},
...
func TestRemoteOverride(t *testing.T) {
	t.Run("load staging override", func(t *testing.T) {
		...
		// Run test
		t.Setenv("Indobase_AUTH_SITE_URL", "http://preview.com")
		t.Setenv("AUTH_SEND_SMS_SECRETS", "v1,whsec_aWxpa2VzdXBhYmFzZXZlcnltdWNoYW5kaWhvcGV5b3Vkb3Rvbw==")
		assert.NoError(t, config.Load("", fsys))
		// Check error
...
func TestGlobFiles(t *testing.T) {
	t.Run("returns seed files matching patterns", func(t *testing.T) {
		// Setup in-memory fs
		fsys := fs.MapFS{
			"Indobase/seeds/seed1.sql":   &fs.MapFile{Data: []byte("INSERT INTO table1 VALUES (1);")},
			"Indobase/seeds/seed2.sql":   &fs.MapFile{Data: []byte("INSERT INTO table2 VALUES (2);")},
			"Indobase/seeds/seed3.sql":   &fs.MapFile{Data: []byte("INSERT INTO table2 VALUES (2);")},
			"Indobase/seeds/another.sql": &fs.MapFile{Data: []byte("INSERT INTO table2 VALUES (2);")},
			"Indobase/seeds/ignore.sql":  &fs.MapFile{Data: []byte("INSERT INTO table3 VALUES (3);")},
		}
		// Mock config patterns
		g := Glob{
			"Indobase/seeds/seed[12].sql",
			"Indobase/seeds/ano*.sql",
		}
		// Run test
		...
		assert.ElementsMatch(t, []string{
			"Indobase/seeds/seed1.sql",
			"Indobase/seeds/seed2.sql",
			"Indobase/seeds/another.sql",
		}, files)
	})
...
func TestLoadFunctionImportMap(t *testing.T) {
	t.Run("uses deno.json as import map when present", func(t *testing.T) {
		config := NewConfig()
		fsys := fs.MapFS{
			"Indobase/config.toml": &fs.MapFile{Data: []byte(`
			project_id = "bvikqvbczudanvggcord"
			[functions.hello]
			`)},
			"Indobase/functions/hello/deno.json": &fs.MapFile{},
			"Indobase/functions/hello/index.ts":  &fs.MapFile{},
		}
		// Run test
		assert.NoError(t, config.Load("", fsys))
		// Check that deno.json was set as import map
		assert.Equal(t, "Indobase/functions/hello/deno.json", config.Functions["hello"].ImportMap)
	})
	...
	t.Run("uses deno.jsonc as import map when present", func(t *testing.T) {
		...
		fsys := fs.MapFS{
			...
			[functions.hello]
			`)},
			"Indobase/functions/hello/deno.jsonc": &fs.MapFile{},
			"Indobase/functions/hello/index.ts":   &fs.MapFile{},
		}
		// Run test
		assert.NoError(t, config.Load("", fsys))
		// Check that deno.jsonc was set as import map
		assert.Equal(t, "Indobase/functions/hello/deno.jsonc", config.Functions["hello"].ImportMap)
	})
	...
	t.Run("config.toml takes precedence over deno.json", func(t *testing.T) {
		...
		assert.Equal(t, "Indobase/custom_import_map.json", config.Functions["hello"].ImportMap)
	})
}
