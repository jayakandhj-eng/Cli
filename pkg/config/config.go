...
//
// > secret = "env(Indobase_AUTH_EXTERNAL_APPLE_SECRET)"
//
...
func NewConfig(editors ...ConfigEditor) config {
	initial := config{baseConfig: baseConfig{
		...
		Realtime: realtime{
			...
			EncryptionKey:   "Indobaserealtime",
			SecretKeyBase:   "EAx3IQ/wRG1v47ZD4NE4/9RzBI8Jmil3x0yhcW4V2NHBP6c2iPIzwjofi2Ep4HIG",
		},
		Storage: storage{
...
func (c *config) Load(path string, fsys fs.FS, overrides ...ConfigEditor) error {
	...
	if err := loadNestedEnv(builder.IndobaseDirPath); err != nil {
		return err
	}
...
func (c *baseConfig) resolve(builder pathBuilder, fsys fs.FS) error {
	...
	for name, tmpl := range c.Auth.Email.Template {
		...
		cwd := filepath.Dir(builder.IndobaseDirPath)
	...
	for name, tmpl := range c.Auth.Email.Notification {
		if len(tmpl.ContentPath) > 0 && !filepath.IsAbs(tmpl.ContentPath) {
			tmpl.ContentPath = filepath.Join(builder.IndobaseDirPath, tmpl.ContentPath)
		}
	...
	for name, bucket := range c.Storage.Buckets {
		...
		if len(bucket.ObjectsPath) > 0 && !filepath.IsAbs(bucket.ObjectsPath) {
			bucket.ObjectsPath = filepath.Join(builder.IndobaseDirPath, bucket.ObjectsPath)
		}
	...
	if len(c.Auth.SigningKeysPath) > 0 && !filepath.IsAbs(c.Auth.SigningKeysPath) {
		c.Auth.SigningKeysPath = filepath.Join(builder.IndobaseDirPath, c.Auth.SigningKeysPath)
	}
	...
	for slug, function := range c.Functions {
		if len(function.Entrypoint) == 0 {
			...
			// Append Indobase/ because paths in configs are specified relative to config.toml
			function.Entrypoint = filepath.Join(builder.IndobaseDirPath, function.Entrypoint)
		}
		if len(function.ImportMap) == 0 {
			...
			function.ImportMap = filepath.Join(builder.IndobaseDirPath, function.ImportMap)
		}
		for i, val := range function.StaticFiles {
			if len(val) > 0 && !filepath.IsAbs(val) {
				function.StaticFiles[i] = filepath.Join(builder.IndobaseDirPath, val)
			}
		}
	...
	if c.Api.Enabled && c.Api.Tls.Enabled {
		if len(c.Api.Tls.CertPath) > 0 {
			c.Api.Tls.CertPath = path.Join(builder.IndobaseDirPath, c.Api.Tls.CertPath)
		}
		if len(c.Api.Tls.KeyPath) > 0 {
			c.Api.Tls.KeyPath = path.Join(builder.IndobaseDirPath, c.Api.Tls.KeyPath)
		}
	}
	...
	if c.Db.Seed.Enabled {
		for i, pattern := range c.Db.Seed.SqlPaths {
			if len(pattern) > 0 && !filepath.IsAbs(pattern) {
				c.Db.Seed.SqlPaths[i] = path.Join(builder.IndobaseDirPath, pattern)
			}
		}
	}
	for i, pattern := range c.Db.Migrations.SchemaPaths {
		if len(pattern) > 0 && !filepath.IsAbs(pattern) {
			c.Db.Migrations.SchemaPaths[i] = path.Join(builder.IndobaseDirPath, pattern)
		}
	}
	return nil
}
...
func (c *config) Validate(fsys fs.FS) error {
	...
	case 12:
		return errors.New("Postgres version 12.x is unsupported. To use the CLI, either start a new project or follow project migration steps here: https://Indobase.com/docs/guides/database#migrating-between-projects.")
	case 13, 14:
	case 15, 17:
		if len(c.Experimental.OrioleDBVersion) > 0 {
			if VersionCompare(c.Experimental.OrioleDBVersion, "15.1.1.13") > 0 {
				c.Db.Image = fmt.Sprintf("Indobase/postgres:%s-orioledb", c.Experimental.OrioleDBVersion)
			} else {
				c.Db.Image = "Indobase/postgres:orioledb-" + c.Experimental.OrioleDBVersion
			}
...
func sanitizeProjectId(src string) string {
	...
	// 63 characters, and we need to save space for padding Indobase_*_edge_runtime.
...
// Ref: https://github.com/Indobase/storage/blob/master/src/storage/limits.ts#L59
...
func (c *tpaClerk) validate() (err error) {
	...
	if !clerkDomainPattern.MatchString(c.Domain) {
		return errors.New("Invalid config: auth.third_party.clerk has invalid domain, it usually is like clerk.example.com or example.clerk.accounts.dev. Check https://clerk.com/setup/Indobase on how to find the correct value.")
	}
