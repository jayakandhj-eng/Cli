...
func TestResolveImports(t *testing.T) {
	t.Run("resolves relative directory", func(t *testing.T) {
		imPath := "Indobase/functions/import_map.json"
		// Setup in-memory fs
		fsys := fs.MapFS{
			...
			"/tmp/":                    &fs.MapFile{},
			"common":                   &fs.MapFile{},
			"Indobase/tests":           &fs.MapFile{},
			"Indobase/functions/child": &fs.MapFile{},
		}
		// Run test
		resolved := ImportMap{}
		...
		assert.Equal(t, "./Indobase/tests", resolved.Imports["parent"])
		assert.Equal(t, "./Indobase/functions/child/", resolved.Imports["child"])
		assert.Equal(t, "./Indobase/missing", resolved.Imports["missing"])
	})
...
func TestResolveDeno(t *testing.T) {
	t.Run("resolves deno.json", func(t *testing.T) {
		imPath := "Indobase/functions/slug/deno.json"
		// Setup in-memory fs
		fsys := fs.MapFS{
			imPath: &fs.MapFile{Data: []byte(`{
				"imports": {
			...
			"Indobase/functions/slug/mod.ts": &fs.MapFile{},
		}
		// Run test
		resolved := ImportMap{}
		...
		// Check error
		assert.NoError(t, err)
		assert.Equal(t, "./Indobase/functions/slug/mod.ts", resolved.Imports["@mod"])
	})
	...
	t.Run("resolves fallback imports", func(t *testing.T) {
		...
		fsys := fs.MapFS{
			...
			"Indobase/import_map.json": &fs.MapFile{Data: []byte(`{
				"imports": {
					"my-mod": "https://deno.land"
				}
			}`)},
	...
	t.Run("throws error on missing import", func(t *testing.T) {
		imPath := "Indobase/functions/slug/deno.jsonc"
		// Setup in-memory fs
		fsys := fs.MapFS{
			imPath: &fs.MapFile{Data: []byte(`{
				"importMap": "../../import_map.json"
