...
const (
	...
	CREATE_VERSION_SCHEMA    = "CREATE SCHEMA IF NOT EXISTS Indobase_migrations"
	CREATE_VERSION_TABLE     = "CREATE TABLE IF NOT EXISTS Indobase_migrations.schema_migrations (version text NOT NULL PRIMARY KEY)"
	ADD_STATEMENTS_COLUMN    = "ALTER TABLE Indobase_migrations.schema_migrations ADD COLUMN IF NOT EXISTS statements text[]"
	ADD_NAME_COLUMN          = "ALTER TABLE Indobase_migrations.schema_migrations ADD COLUMN IF NOT EXISTS name text"
	INSERT_MIGRATION_VERSION = "INSERT INTO Indobase_migrations.schema_migrations(version, name, statements) VALUES($1, $2, $3)"
	...
	DELETE_MIGRATION_VERSION = "DELETE FROM Indobase_migrations.schema_migrations WHERE version = ANY($1)"
	DELETE_MIGRATION_BEFORE  = "DELETE FROM Indobase_migrations.schema_migrations WHERE version <= $1"
	TRUNCATE_VERSION_TABLE   = "TRUNCATE Indobase_migrations.schema_migrations"
	SELECT_VERSION_TABLE     = "SELECT version, coalesce(name, '') as name, statements FROM Indobase_migrations.schema_migrations"
	LIST_MIGRATION_VERSION   = "SELECT version FROM Indobase_migrations.schema_migrations ORDER BY version"
	CREATE_SEED_TABLE        = "CREATE TABLE IF NOT EXISTS Indobase_migrations.seed_files (path text NOT NULL PRIMARY KEY, hash text NOT NULL)"
	UPSERT_SEED_FILE         = "INSERT INTO Indobase_migrations.seed_files(path, hash) VALUES($1, $2) ON CONFLICT (path) DO UPDATE SET hash = EXCLUDED.hash"
	SELECT_SEED_TABLE        = "SELECT path, hash FROM Indobase_migrations.seed_files"
)

// TODO: support overriding `Indobase_migrations.schema_migrations` with user defined <schema>.<table>
