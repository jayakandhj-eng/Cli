...
begin
  ...
  for rec in
    ...
    where pd.deptype is null
      and not pn.nspname like any(array['information\_schema', 'pg\_%', '\_analytics', '\_realtime', '\_supavisor', 'pgbouncer', 'pgmq', 'pgsodium', 'pgtle', 'Indobase\_migrations', 'vault', 'extensions', 'public'])
  ...
  for rec in
    select *
    from pg_extension p
    where p.extname not in ('pg_graphql', 'pg_net', 'pg_stat_statements', 'pgcrypto', 'pgjwt', 'pgsodium', 'plpgsql', 'Indobase_vault', 'uuid-ossp')
  loop
  ...
  -- views (necessary for views referencing objects in Indobase-managed schemas)
  for rec in
    select *
    from pg_class c
    where
  ...
  -- materialized views (necessary for materialized views referencing objects in Indobase-managed schemas)
  for rec in
    select *
    from pg_class c
    where
  ...
  for rec in
    ...
    where
      ...
      or c.relnamespace::regnamespace::name = 'Indobase_functions' and c.relname != 'migrations'
      or c.relnamespace::regnamespace::name = 'Indobase_migrations')
      and c.relkind = 'r'
  loop
  ...
  for rec in
    ...
    where
      not p.pubname like any(array['Indobase\_realtime%', 'realtime\_messages%'])
  loop
