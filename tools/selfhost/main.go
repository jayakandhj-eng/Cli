...
func updateSelfHosted(ctx context.Context, branch string) error {
	...
	master := "master"
	if err := shared.CreateGitBranch(ctx, client, Indobase_OWNER, Indobase_REPO, branch, master); err != nil {
		return err
	}
	...
	return shared.CreatePullRequest(ctx, client, Indobase_OWNER, Indobase_REPO, pr)
}
...
func updateComposeVersion(ctx context.Context, client *github.Client, path, ref string, stable map[string]string) error {
	...
	file, _, _, err := client.Repositories.GetContents(ctx, Indobase_OWNER, Indobase_REPO, path, &opts)
	if err != nil {
		return err
	}
	...
	resp, _, err := client.Repositories.UpdateFile(ctx, Indobase_OWNER, Indobase_REPO, path, &commit)
	if err != nil {
		return err
	}
