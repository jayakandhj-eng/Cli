...
func TestBranchValidation(t *testing.T) {
	...
	t.Run("branch not a directory", func(t *testing.T) {
		...
		path := "/Indobase/.branches/test-branch"
...
func TestBranchCreation(t *testing.T) {
	...
	t.Run("docker attach failure", func(t *testing.T) {
		...
		assert.Error(t, err)
		errText := err.Error()
		assert.True(t, strings.Contains(errText, "unable to upgrade to tcp, received 404") || strings.Contains(errText, "cannot connect to the Docker daemon"))
		assert.Empty(t, apitest.ListUnmatchedRequests())
	})
}
